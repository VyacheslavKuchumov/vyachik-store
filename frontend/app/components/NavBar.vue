<template>
  <header class="border-b border-gray-200 shadow-sm :dark:border-gray-700">
    <nav class="container mx-auto flex items-center justify-between py-4 px-6">
      <h1 class="text-xl font-bold" @click="$router.push('/')">Vyachik's Store 💵</h1>
...
const items = computed<NavigationMenuItem[][]>(() => {
  ...
  const baseItems: NavigationMenuItem[][] = [
    [
      ...
      { label: 'Каталог', icon: 'i-lucide-list-collapse', to: '/browse' },
      { label: 'О нас', icon: 'i-lucide-info', to: '/about' },
    ]
  ]
  ...
  if (auth.isAuthenticated) {
    return [
      ...
      [
        ...
        { label: 'Выйти', icon: 'i-lucide-log-out', action: 'logout' }
      ]
    ]
  } else {
    return [
